...
def join_all_dataframes(season):
    # Reading dataframes
    ...
    df_ucr_season = pd.read_csv("cleaned_data/{}_cleaned.csv".format(season), index_col=[0])
    ...
    crime_columns = df_ucr_season.loc[:, "M0_9":"JN"].columns.tolist()
    ...
    grouped_ucr_2015_age = df_ucr_season.groupby(by=["MONTH", "metfips", "YEAR", "city_ascii", "SUB"], as_index=False)[arrests_columns].sum()

    # Melting ucr to long format
    grouped_ucr_2015_age = grouped_ucr_2015_age.melt(id_vars=["MONTH", "metfips", "YEAR", "city_ascii", "SUB"], value_name="age_arrests", var_name="age_range")

    # Creating dataframe with total arrests
    total_arrests = grouped_ucr_2015_age.groupby(by=["MONTH", "metfips", "YEAR", "city_ascii", "SUB"], as_index=False)["age_arrests"].sum()
    total_arrests.columns = ["MONTH", "metfips", "YEAR", "city_ascii", "SUB", "total_arrests"]

    # Merging crime dataframes
    df_ucr_2015 = pd.merge(grouped_ucr_2015_age, total_arrests, how="inner", on=["MONTH", "metfips", "YEAR", "city_ascii", "SUB"])
    df_ucr_2015.columns = ["month","metfips","year","city_ascii","sub","age_range","age_arrests","total_arrests"]

    # Merging all dataframes
    df_final = pd.merge(df_cps_nba, df_ucr_2015, how="inner", on=["month", "year", "metfips"])
    df_final.drop(["YEAR", "METFIPS", "Unnamed: 29"], inplace=True, axis=1)
